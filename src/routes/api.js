...
const responseLogin = require("../dto/login");
...
router.post('/oauth/login/google/callback', async (req, res) => {
    if (!req.body) {
        return res.json(responseLogin(false, null, `Missing user data: ${req.body}`));
    }
    const user = req.body;
    const googleUser = user.isGoogleLogin ?? false
    if (googleUser === true) {
        const users = await userApi.queryUserTable()
        // if we have some users, and we find a match in BQ, allow the user to login
        if (users && users.length > 0) {
            // hardcode google login users - for now
            return res.json(responseLogin(true, {'name': user.name, 'email': user.email}, null))
        }
    }
    res.json(responseLogin(false, null, `No user registered for name ${user.name}`));
});
